...
`quality-agent-cockpit.html` 是在原有工业风格POC基础上改出的面试/作品集版本，定位为“面向真实生产场景的产品交互与逻辑验证载体”。它不是单纯展示页，也不声称已接入真实产线；页面重点展示质检员在真实使用场景中会看到的界面，以及背后本地Agent/Workflow的运行链路。

当前已升级为本地前后端联动版本：用 `server.mjs` 启动本地服务后，浏览器点击“合格批次”或“异常批次”会请求 `/api/run-agent`，由后端优先读取 SQLite 样例库 `local-agent/db/quality-agent-demo.sqlite` 并调用 `local-agent/agent.mjs`，再把真实返回的质量判定、预测指标、风险证据和处置建议回填到驾驶舱。若数据库不可用，后端才会降级读取 `local-agent/samples/` 中的JSON样例。

数据库层已补齐：`local-agent/db/quality-agent-demo.sqlite` 是POC样例库，包含质量规则上限/下限、生产批次、视觉特征、电流特征、配比参数、上下文信息和电流时序点。它用于展示字段设计和本地数据链路，不代表真实客户生产数据库。

页面新增内容：

- 顶部可直接选择“合格批次”或“异常批次”，方便录屏和截图。
- 中央流程条改为输入、规则、视觉、电流、预测、GLM、HITL、台账。
- 右侧新增Agent链路运行轨迹，展示输入节点、规则节点、视觉/电流特征、GLM研判、HITL确认和台账归档。
- 正常批次对齐本地Agent输出：坍落度185mm、扩展度461mm、倒坍时间5.3s、浆体富裕度17.9%。
- 异常批次对齐本地Agent输出：坍落度129mm、扩展度352mm、倒坍时间8.2s、浆体富裕度12.0%，并进入HITL确认。
- 异常批次未完成授权调整前不能直接放行，避免把方案说成无边界全自主控制。
